            ...
            <div className="flex flex-wrap -mx-4 mb-12 pb-8">
                { articles.map((article, index) => (
                    <div className="w-full md:w-1/2 lg:w-1/3 py-2 px-4" key={ article.node.id }>
                        <h3>
                            <ArticleLink className={ `text-white light-mode:text-gray-800 p-1 text-lg border-b border-e6n-${colors[index % 4]} hover:bg-e6n-${colors[index % 4]}` } href={ article.node.url } title={ article.node.title } target="_blank">{ article.node.title }</ArticleLink>
                        </h3>
